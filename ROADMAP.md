...
- **Schema catalog & introspection** — `sqlite_schema`/`sqlite_master` readable;
  ...
  surface; `EXPLAIN QUERY PLAN` shaping (incl. `SCAN CONSTANT ROW` — and, for a
  `FROM`-less SELECT carrying one or more non-correlated scalar subqueries in a single
  position, `SELECT (SELECT count(*) FROM u)` → `SCAN CONSTANT ROW#SCALAR SUBQUERY 1#
  <body>`, declining the reverse-numbered cross-position / `EXISTS` / `IN (SELECT)`
  forms; and an
  ...
  *CTE body* — `WITH c AS (VALUES(1,2),(3,4)) SELECT * FROM c` — materializes as
  ...
  re-deriving their scans) each decline cleanly; an *aggregate* or *DISTINCT*
  CTE/derived body over a single base table — `SELECT * FROM (SELECT count(*) FROM t)`,
  `SELECT * FROM (SELECT DISTINCT b FROM t)`, `WITH c AS (SELECT count(*) FROM t)
  SELECT * FROM c`, a `CREATE VIEW` whose body aggregates — materializes the same way
  (`CO-ROUTINE {s|c}` whose child is the body's own aggregate / DISTINCT plan, then the
  outer `SCAN`/`SEARCH`), where it previously declined; a join / nested-subquery body
  or such a source combined with another table still decline; an aggregate
  ...
  *secondary* index column (`a = 1 OR a = 2 OR …`, every disjunct a bare equality on
  ...
  Symmetrically, `col IS <non-null constant>` is now a *seekable equality*: SQLite's
  `IS` behaves exactly like `=` for non-NULL operands (a NULL `col` makes both false),
  so `col IS 5` seeks the same index/rowid key as `col = 5` — graphite previously
...
- **B9g — eq-prefix + trailing rowid range on a secondary index.** `WHERE b=? AND
  rowid>?` reads `SEARCH … USING INDEX ib (b=? AND rowid>?)` in SQLite (the index's
  implicit trailing rowid is bounded); graphite renders only `(b=?)`. Extends the
  existing composite eq-prefix + next-column range seek to the rowid tail.
...
- **B1b — cost-based join reordering.** graphite's per-cursor seek/bloom-filter
  ...
  B9h is the single-table sibling — same regression-risk profile.
