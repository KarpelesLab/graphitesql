...
- **Aggregate/window extras** — ✅ `FILTER (WHERE …)` on aggregates. *Remaining:*
  the `WINDOW name AS (…)` clause and named-window reuse, frame `EXCLUDE`,
  `percent_rank`/`cume_dist`, `GROUP_CONCAT` ordering, `FILTER` on window
...
- **Indexing breadth** — ✅ partial indexes (`CREATE INDEX … WHERE`).
  *Remaining:* expression indexes (`CREATE INDEX … (lower(x))`), `DESC` index
  columns honored in seeks, partial-index use in the planner (currently scan-only),
