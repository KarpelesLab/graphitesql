...
//! compile-time emit/suppress decision and `DISTINCT` is a no-op — all three now
//! run on the VDBE instead of deferring. The row is suppressed when `LIMIT` is
//! exactly 0 or a positive `OFFSET` skips past it; a negative `LIMIT` is unlimited
//! and a non-positive `OFFSET` skips nothing, matching SQLite. (`ORDER BY` over a
//! FROM-less select still defers — its term validation is left to the tree-walker.)
//!
