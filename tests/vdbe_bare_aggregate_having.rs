...
//! the aggregate compiler now filters its single output row when the HAVING's
//! aggregate(s) also appear in the projection (bound to their finalized slots).
//! A HAVING aggregate that is NOT in the projection still defers (a "misuse of
//! aggregate" during compile), which the tree-walker evaluates. Either way the
//! result must match the real `sqlite3` CLI.
...
fn bare_aggregate_having_matches_sqlite3() {
    ...
    check(
        ...
        &[
            ...
            // HAVING aggregate not in the projection -> defers, still correct
