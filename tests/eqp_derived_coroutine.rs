...
fn non_flattenable_outer_shapes_decline() {
    ...
    // a *narrower* / outer-`ORDER BY` outer over a `LIMIT` body needs separate merge
    // handling — all decline cleanly rather than mis-render. (An inner *aggregate* /
    // *DISTINCT* body, and a non-flattenable `LIMIT`/`OFFSET` body, render as a
    // CO-ROUTINE; a bare-`LIMIT` body under a pure-wildcard outer flattens — see
    // `tests/eqp_aggregate_coroutine.rs` and
