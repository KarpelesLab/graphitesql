...
//! collation. The single-table scan path threads the per-column collation into
//! `DistinctCheck` (resolved with the same `explicit_collation`/`col_collation`
//! logic as an ORDER BY key), so a column with a *declared* `NOCASE`/`RTRIM`
//! collation — and an explicit `COLLATE BINARY` — runs on the VDBE. An explicit
//! *non-BINARY* `COLLATE` on a projection still defers to the tree-walker via the
//! `projections_have_explicit_collation` guard (the other DISTINCT paths — group,
//! join, aggregate — likewise still defer any non-BINARY collation). Verified
