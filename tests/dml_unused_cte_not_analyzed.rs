...
fn dml_unused_cte_not_analyzed_parity() {
    ...
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH u AS (SELECT nope) UPDATE t SET a=2; SELECT a FROM t;");
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH u AS (SELECT nope) DELETE FROM t WHERE a=1; SELECT count(*) FROM t;");
    // An unreferenced bad *table* inside the CTE is likewise ignored.
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH u AS (SELECT * FROM nosuchtbl) UPDATE t SET a=5; SELECT a FROM t;");

    // An unused infinite recursive CTE is never run.
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH RECURSIVE r(n) AS (SELECT 1 UNION ALL SELECT n+1 FROM r) UPDATE t SET a=9; SELECT a FROM t;");

    // A *used* CTE with a bad column still errors (regression guards).
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH u AS (SELECT nope AS x) UPDATE t SET a=(SELECT x FROM u);");
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH u AS (SELECT nope AS x) DELETE FROM t WHERE a IN (SELECT x FROM u);");

    // A used CTE produces the right result through every reference site.
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1),(2),(3); WITH u(k) AS (VALUES(2),(3)) DELETE FROM t WHERE a IN (SELECT k FROM u); SELECT a FROM t ORDER BY a;");
    same("CREATE TABLE t(a,b); INSERT INTO t VALUES(1,0); WITH u(k,v) AS (VALUES(1,7)) UPDATE t SET b=u.v FROM u WHERE t.a=u.k; SELECT a,b FROM t;");
    same("CREATE TABLE t(a,b); INSERT INTO t VALUES(1,0); WITH u AS (SELECT 5 v) UPDATE t SET b=(SELECT v FROM u); SELECT a,b FROM t;");
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH u AS (SELECT 9 v) UPDATE t SET a=2 RETURNING (SELECT v FROM u);");
    same("CREATE TABLE t(a,b); INSERT INTO t VALUES(1,1); WITH u AS (SELECT 7 p,8 q) UPDATE t SET (a,b)=(SELECT p,q FROM u); SELECT a,b FROM t;");
    ...
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH v AS (SELECT bad), u AS (SELECT * FROM v) UPDATE t SET a=3; SELECT a FROM t;");
    same("CREATE TABLE t(a); INSERT INTO t VALUES(1); WITH v AS (SELECT bad x), u AS (SELECT x FROM v) UPDATE t SET a=(SELECT x FROM u);");
}
