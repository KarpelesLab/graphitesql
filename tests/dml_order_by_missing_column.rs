...
/// `UPDATE/DELETE … ORDER BY … LIMIT` is the SQLite update/delete-limit
/// extension, compiled in only when the amalgamation is built with
/// `SQLITE_ENABLE_UPDATE_DELETE_LIMIT`. graphite always supports it, but a
/// stock `sqlite3` (including CI's pinned 3.50.4) is built *without* it and
/// reports `near "ORDER": syntax error` for the whole family — so the
/// differential comparison is only meaningful against a build that has the
/// extension. Probe it with a *valid* statement and skip when it is absent.
    ...
    if !sqlite3_has_update_delete_limit() {
        eprintln!(
            "sqlite3 built without SQLITE_ENABLE_UPDATE_DELETE_LIMIT; \
             skipping the differential UPDATE/DELETE ORDER BY LIMIT comparison"
        );
        return;
    }
